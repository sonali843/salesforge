...
const Leads = () => {
  ...
  // Selected lead IDs for multiple deletion
  const [selected, setSelected] = useState([]);

  // Multiple delete loading state
  ...
  const remove = async (id) => {
    ...
    try {
      ...
      // If this was the last item on a page, go back one page.
  ...
  const removeSelected = async () => {
    ...
    const confirmed = confirm(
      `Are you sure you want to delete ${idsToDelete.length} selected lead${
    ...
    try {
      // The project already exposes POST /leads/bulk-delete.
      // Send all selected IDs in ONE request instead of firing many DELETE requests.
      const result = await leadService.bulkDelete(idsToDelete);
      const deletedCount = Number(result?.deleted ?? idsToDelete.length);
      ...
      toast.success(
        `${deletedCount} lead${deletedCount > 1 ? "s" : ""} deleted successfully`
      );
    } catch (e) {
      ...
      <section>
        <div className="flex items-center justify-between mb-3">
          ...
          {isMember && selected.length > 0 && (
            ...
            >
              <span
                className={`text-sm font-medium ${body}`}
              >
                        ...
                        >
                          {isMember && (
                            <button
                              ...
                              className="inline-flex items-center gap-1 text-red-500 hover:text-red-700 text-xs"
                            >
